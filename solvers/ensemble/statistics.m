function [fname,para,state] = statistics(vars,falgo,npara,flist,fRBM,runs,T,tw,monitor)
...
E = zeros(tots,nr); Et = zeros(tots,nr,rrecs); dlap = zeros(tots,nr,recs); 
...
E = zeros(tots,2,nr); Et = zeros(tots,2,nr,rrecs); dlap = zeros(tots,2,nr,recs); 
q2 = zeros(tots,nr); q4 = zeros(tots,nr);
cor = cell(1,tots); con = cell(1,tots); cort = cell(1,tots); cont = cell(1,tots);
...
Et = zeros(tots,recs); bclus = cell(1,tots); 
...
Et(tot,:) = st.Et; bclus{tot} = st.bclus; 
